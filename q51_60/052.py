...
if __name__ == '__main__':
    ...
    out = lib4150.Opening_operation(img2, 3)
    cv2.imshow("imori", out)
    cv2.waitKey(0)
    cv2.imwrite("q51_60/052a.jpg", out)
    cv2.destroyAllWindows()

    out = lib5160.Tophat_transform(img2).clip(0,255).astype(np.uint8)
    cv2.imshow("imori", out)
    cv2.waitKey(0)
    cv2.imwrite("q51_60/052.jpg", out)
    cv2.destroyAllWindows()