...
if __name__ == '__main__':
    ...
    _, rects, scores = lib91100.my_object_detection((train_img, gt, 60, 60, 200, 0, 32, 32), (test_img, sizes, 4, 4, 32, 32), (0.01, 14, 10000), 0.7, 8)
    rects, scores = lib91100.Object_detection3_nms(test_img, rects, scores, 0.25)
    
    test_gt = np.array(((27, 48, 95, 110), (101, 75, 171, 138)), dtype=np.float32)
    out = lib91100.Object_detection5_evaluate(test_img, rects, scores, test_gt, 0.5)
    
    cv2.imshow("imori", out)
    cv2.waitKey(0)
    cv2.imwrite("q91_100/100.jpg", out)
